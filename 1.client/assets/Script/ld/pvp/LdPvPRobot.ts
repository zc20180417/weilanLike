...
export class LdPvPRobot {


    private _gameCtrl:LdPvpGameCtrl;
    ...
    constructor(ctrl:LdPvpGameCtrl) {
        this._gameCtrl = ctrl;
    }
    ...
    */
    ...
    private tryCallHero() {
        ...
        GameEvent.emit(EventEnum.TRY_CALL_TOWER , ECamp.RED);
    }

    private tryStrengthenSkill() {
        GameEvent.emit(EventEnum.TRY_STRENGTHEN , ECamp.RED);
        const skills = Game.curLdGameCtrl.getHeroBuildingCtrl(ECamp.RED).getReadyStrengthSkills();
        if(skills.length == 0) return;
        GameEvent.emit(EventEnum.LD_ACTIVE_STRENGTH_SKILL , skills[0].heroId , skills[0].skillID , ECamp.RED);
    }

    private onActiveMaxStrengthSkill(heroId:number , campId:ECamp) {
        if (campId != ECamp.RED) return;
        const skills = Game.curLdGameCtrl.getHeroBuildingCtrl(ECamp.RED).getReadyStrengthSkills(heroId);
        if(skills.length == 0) return;
        GameEvent.emit(EventEnum.LD_ACTIVE_STRENGTH_SKILL , skills[0].heroId , skills[0].skillID , ECamp.RED);
    }
