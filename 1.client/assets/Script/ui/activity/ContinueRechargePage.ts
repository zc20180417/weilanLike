...
@ccclass
@menu("Game/ui/active/ContinueRecharge/ContinueRechargePage")
export default class ContinueRechargePage extends TapPageItem {
    ...
    private updateActiveData(nid:number) {
        if (nid == ACTIVE_TYPE.CONTINUE_RECHARGE) {
            this.refresh();
        }
    }
    ...
    private onTimer() {
        if (this._cfg) {
            let time = Game.sysActivityMgr.getActiveRestTime(ACTIVE_TYPE.CONTINUE_RECHARGE);
    ...
    public refresh() {
        this._cfg = Game.sysActivityMgr.getActivityInfo(ACTIVE_TYPE.CONTINUE_RECHARGE);
        this._privateData = Game.sysActivityMgr.getPrivateData(ACTIVE_TYPE.CONTINUE_RECHARGE);
