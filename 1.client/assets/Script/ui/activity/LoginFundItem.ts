...
import { GS_SceneBattlePass2Config_PassItem } from "../../net/proto/DMSG_Plaza_Sub_BattlePass";
...
@ccclass
@menu('Game/ui/active/LoginFundItem')
export class LoginFundItem extends BaseItem {
    ...
    public setData(data:any , index?:number) {
        ...
        if (!this.data) return;
        let itemData = data.itemData as GS_SceneBattlePass2Config_PassItem;
        if (itemData.nwarid > 0) {
            this.titleLabel.string = `通关第${itemData.nwarid}关`;
        } else if (itemData.nday > 0) {
            this.titleLabel.string = `第${itemData.nday}天`;
        }

        let progressValue = Utils.getDiffDay(data.config.nstarttime * 1000 , Math.floor(GlobalVal.getServerTime()));
        this.progressLabel.string = `（${StringUtils.fontColor(progressValue + "" , "#d33441" )}/${itemData.nday}）`;
    ...
    onClick() {
        ...
        Game.battlePassMgr.battlePass2GetReward(this.data.config.nid , this.index , 0);
    }
